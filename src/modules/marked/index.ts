...
export function markedSetup(): void
{
  marked.setOptions({
    gfm: true,
    breaks: true,
...
export function baseRenderer(renderer?: Renderer): Renderer
{
  if (!renderer) renderer = new Renderer()
  renderer.heading = (text: string, level: any) => {
...
export function checklistRenderer(renderer?: Renderer): Renderer
{
  if (!renderer) renderer = new Renderer()
